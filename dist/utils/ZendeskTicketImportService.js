...
const safeAxios = (url_1, ...args_1) => __awaiter(void 0, [url_1, ...args_1], void 0, function* (url, options = {}, retries = 5) {
    var _a;
    while (retries > 0) {
        try {
            // ✅ GLOBAL THROTTLE (200ms gap)
            const now = Date.now();
            const diff = now - lastRequestTime;
            if (diff < 200) {
                yield sleep(200 - diff);
            }
            ...
            try {
                ...
                while (url) {
                    ...
                    for (const t of tickets) {
                        ...
                        const exists = yield prisma.tickets.findFirst({
                            where: { zendesk_ticket_id: Number(t.id) },
                        });
                        if (exists)
                            continue;
            ...
            try {
                ...
                for (const c of comments) {
                    ...
                    if ((_g = c.attachments) === null || _g === void 0 ? void 0 : _g.length) {
                        const attachmentLimit = (0, p_limit_1.default)(1);
