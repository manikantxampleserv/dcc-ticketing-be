...
class EmailService {
    ...
    initializeTransporter() {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            try {
                const emailConfiguration = yield this.loadEmailConfiguration();
                // Determine if using Gmail service or custom SMTP
                const isGmailService = ((_a = emailConfiguration.smtp_server) === null || _a === void 0 ? void 0 : _a.includes("gmail.com")) ||
                    ((_b = emailConfiguration.username) === null || _b === void 0 ? void 0 : _b.includes("@gmail.com"));
                if (isGmailService) {
                ...
                if (this.transporter) {
                    ...
                    console.log(`✅ Email transporter initialized successfully for logInst: ${this.logInst}`);
                }
            }
            catch (error) {
                console.error(`❌ Failed to initialize email transporter for logInst ${this.logInst}:`, error);
                this.transporter = null;
                throw error;
            }
        });
    ...
    loadEmailConfiguration() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const emailConfiguration = yield prisma.email_configurations.findFirst({
                    where: {
                        log_inst: 1,
                    },
                });
                if (!emailConfiguration) {
                    throw new Error(`No active email configuration found for logInst: ${this.logInst}`);
                }
                ...
                console.error(`❌ Error loading email configuration for logInst ${this.logInst}:`, error);
                throw error;
            }
        });
    }
    ...
    ensureTransporter() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.transporter) {
                yield this.initializeTransporter();
            }
            if (!this.transporter) {
    ...
    sendCommentEmailToCustomer(ticket, comment, additionalEmails) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                // const imageUrl = comment?.image_url || comment?.imageUrl;
                const imageUrls = Array.isArray(comment === null || comment === void 0 ? void 0 : comment.imageUrls)
                    ? comment.imageUrls
                    : [];
                ...
                for (let i = 0; i < imageUrls.length; i++) {
                    ...
                    try {
                        ...
                        const fileName = imageUrl.split("/").pop() || `ticket_${ticket.id}_${i + 1}.jpg`;
  ...
  `;
                ...
                if (comment.id) {
                    yield prisma.ticket_comments.update({
                        where: { id: comment.id },
                        data: { email_message_id: newMessageId },
                    });
                }
                console.log(`✅ Email sent to customer ${customerEmail} for ticket #${ticket.ticket_number}`);
           ...
           <table style="width: 100%; border-collapse: collapse;">
             <tr>
               <td style="vertical-align: top;">
                 <strong style="color: #333; font-size: 16px;">Ticket #${ticket.ticket_number}</strong><br>
                ...
                <h3 style="font-size:16px; color:#333333; margin:0;">
                  📋 Conversation History (${previousComments.length})
                    ...
                    previousComments === null || previousComments === void 0 ? void 0 : previousComments.forEach((pc) => {
                        ...
                        const { border, bg } = this.getBorderColors(isCustomer, sourceIsEmail);
                        html += `
            <tr>
                ...
                <table width="100%" cellpadding="0" cellspacing="0"
                  ...
                  <tr>
                    ...
                    <div style="display:flex;align-items:start;">
                     ${((_a = pc.ticket_comment_users) === null || _a === void 0 ? void 0 : _a.avatar)
                     ...
                     <div style="margin-top:8px; font-size:14px; color:#333333; line-height:1.5;">
  ${(_b = pc.comment_text) !== null && _b !== void 0 ? _b : ""
                     ...
                     ${pc.attachment_urls
                            ? (() => {
                                const urls = JSON.parse(pc.attachment_urls || "[]");
                                return urls
                                    .map((url) => `
    ...
    sendTicketCreationEmailToCustomer(ticket, customerEmail) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            try {
                // ✅ Ensure transporter is initialized
                ...
                // Generate unique message ID for this outgoing email
                const newMessageId = `<ticket-${ticket.id}-created-${Date.now()}@gmail.com>`;
                // For new tickets, this will be the thread starter
                const threadId = ticket.email_thread_id || newMessageId;
                const htmlContent = yield this.ticketCreationEmailTemplate(ticket);
                const mailOptions = {
                    from: `"Support Team" <${process.env.SMTP_USERNAME}>`,
                    to: [(_a = ticket === null || ticket === void 0 ? void 0 : ticket.agents_user) === null || _a === void 0 ? void 0 : _a.email, customerEmail],
                    subject,
                    html: htmlContent,
                ...
                if ((ticket === null || ticket === void 0 ? void 0 : ticket.cc_of_ticket) && ticket.cc_of_ticket.length > 0) {
                    mailOptions.cc = ticket.cc_of_ticket
                        .map((cc) => cc.email)
                        .join(",");
                }
                yield this.transporter.sendMail(mailOptions);
                console.log(`✅ Ticket creation email sent to ${customerEmail} for ticket #${ticket.ticket_number}`);
                return {
                    messageId: newMessageId,
                    threadId: threadId,
                };
    ...
    ticketCreationEmailTemplate(ticket) {
        return __awaiter(this, void 0, void 0, function* () {
                  ...
                  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" class="mobile-stack">
                    <tr>
                      <td style="padding-bottom: 12px; vertical-align: top;" class="mobile-spacing">
                        ...
                        <div class="mobile-font-medium" style="color: #1A1A1A; font-size: 20px; font-weight: 700; margin-top: 4px;">#${ticket.ticket_number}</div>
                      </td>
              ...
              <td style="padding: 0 30px 30px 30px;" class="mobile-padding">
                <div style="background-color: #FFF9E6; border-radius: 6px; padding: 20px; border-left: 4px solid #FF8C00;" class="mobile-padding-small">
                  ...
                  <p class="mobile-font-small" style="color: #333333; font-size: 14px; line-height: 1.6; margin: 0; word-wrap: break-word;">
                    Simply reply to this email with additional details. Please keep the ticket number <strong>#${ticket.ticket_number}</strong> in the subject line to ensure your message is properly tracked.
                  </p>
                </div>
              </td>
            </tr>
